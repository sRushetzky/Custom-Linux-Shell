...
int main(int argc, const char* argv[]) {
    ...
    while (1) {  // Start an infinite loop to keep the shell running
        ...
        if (strcmp(args[0], "echo") == 0) {
            int redirectIndex = -1; // Index of the redirection symbol (if any)
            int redirectType = 0; // Redirection type (1 for '>', 2 for '>>')
            ...
            if (redirectIndex != -1) {
                char* filename = args[redirectIndex + 1];
                if (filename == NULL) {
                ...
                char output[MAX] = "";
                for (int j = 1; j < redirectIndex; j++) {
                    strcat(output, args[j]); // Concatenate arguments into output string
                    if (j < redirectIndex - 1) {
                        strcat(output, " "); // Add space between arguments
                    }
                }

                write(fd, output, strlen(output)); // Write output string to file
